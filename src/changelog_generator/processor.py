...
def process_repository(
    ...
    """
    Fetches repository data based on tags, prepares it, and generates a changelog using LLM.
    ...
    try:
        ...
        # No need to pre-format data here anymore
        # llm_input_data = format_commit_data_for_llm(commits, files) # Removed

        llm = LLMHandler()  # Assumes GOOGLE_API_KEY is set in env
        changelog_content = llm.generate_changelog(
            commits=commits,
            files_changed=files, # Pass the raw files list
            from_ref=base_ref,
            to_ref=head_ref,
            repo=repo
        )
        ...
        print("\n--- Generated Changelog ---")
        # The actual content is now generated by the LLM
        # print(llm_input_data) # Removed
